...
public abstract class DraggableObject : MonoBehaviour {
    ...
    public void OnTriggerStay2D(Collider2D other)
    {
        ...
        var bodyPart = other.gameObject.GetComponent<BodyPartObject>();

        if (otherObj != null && 
            (CurrentState != ObjectState.Pressed && CurrentState != ObjectState.Transformed) &&
            (otherObj.CurrentState == ObjectState.Pressed || otherObj.CurrentState == ObjectState.Transformed ||
                otherObj.CurrentState == ObjectState.Launched))
        {
        ...
        {
            //var otherCenter = new Vector2(bodyPart.transform.position.x + bodyPart.transform. rect.width, bodyPart.transform.position.y + bodyPart.transform.rect.height)
            var cam = Camera.main;
            var otherCenter = (Vector2) cam.WorldToScreenPoint(bodyPart.Center);

            var direction = (GetCenter() - bodyPart.Center);
            direction.Normalize();

            //Debug.DrawLine(bodyPart.Center, new Vector3(0,0,0), Color.red, 5f, false);
            //Debug.DrawLine(GetCenter(), new Vector3(0, 0, 0), Color.green, 5f, false);
            //Debug.DrawLine(bodyPart.Center, GetCenter(), Color.white, 5f, false);
